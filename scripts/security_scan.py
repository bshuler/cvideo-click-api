...
def run_bandit_scan() -> Dict[str, Any]:
    ...
    # Run bandit with JSON output on src/, scripts/, and tests/ directories
    scan_dirs = []
    ...
    if not scan_dirs:
        return {
            "tool": "bandit",
            "status": "skipped",
            "reason": "No Python source directories found (src/, scripts/, tests/)",
            "issues": 0,
        }
...
def run_checkov_scan() -> Dict[str, Any]:
    ...
    if not iac_files:
        return {
            "tool": "checkov",
            "status": "skipped",
            "reason": "No infrastructure files found (.tf, .yaml, .yml)",
            "issues": 0,
        }

    # Run checkov on found files
    cmd = [
        "checkov",
        "-f",
        *iac_files[:5],  # Limit to first 5 files to avoid long runs
        "-o",
        "json",
        "--quiet",
        "--compact",
    ...
    # Add checkov config if it exists
    if Path(".checkov.yml").exists():
        cmd.extend(["--config-file", ".checkov.yml"])
    ...
    try:
        if stdout:
            ...
            if isinstance(result, dict):
                ...
                # Multiple framework results - process each section
                for section in result:
                    if isinstance(section, dict) and "results" in section:
                        section_failed = section.get("results", {}).get(
                            "failed_checks", []
                        )
                        failed_checks.extend(section_failed)
            else:
                failed_checks = []
            ...
            if issues > 0:
                ...
                if not QUIET_MODE:
                    for check in failed_checks[:5]:  # Show first 5 issues
                        file_path = check.get(
                            "file_path", check.get("repo_file_path", "unknown")
                        )
                        ...
                        print(f"   {file_path}:{line_range} - {check_id}")
                        print(f"   {check_name}")
...
def run_secret_detection() -> Dict[str, Any]:
    ...
    secret_patterns = {
        ...
        "api_key": (
            r'[aA][pP][iI]_?[kK][eE][yY].*[=:]\s*["\']?' r'[a-zA-Z0-9_\-]{16,}["\']?'
        ),
...
def check_gitignore() -> Dict[str, Any]:
    ...
    issues = []
    # Comprehensive security-critical patterns
    required_patterns = [
        ...
        "*credentials*",
        "env-vars.json",
        # Terraform security-critical files
        "*.tfstate",
        "*.tfstate.*",
        ".terraform/",
        "terraform.tfvars",
